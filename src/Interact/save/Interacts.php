...
class Interacts extends \Table
{
    ...
    {
        parent::__construct($course, 'interact');
    }
...
SQL;
	 ...
	 * @param $time Time stamp in Unix time
	 * @return string ID for the inserted interaction
	 */
    public function add(Interaction $interaction, $time) {
        $pdo = $this->pdo();

        $sql = <<<SQL
insert into $this->tablename(userid, assigntag, sectiontag, time, type, pin, private, summary, message)
values(?, ?, ?, ?, ?, ?, ?, ?, ?)
SQL;

		$a = array($interaction->get_user_id(),
...
SQL;
    ...
    public function alter() {
        ...
        if($this->index_exists($indexname)) {
            return true;
        }
