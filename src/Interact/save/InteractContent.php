 ...
 * Base class for Interact system content: interaction and discussion items
 *
 * This class implements a simple structure for saving content in the table as
 * XML with an edit history. It also supports these common elements of interaction
...
abstract class InteractContent {
     ...
     * @param \User|null $user The user or null if not yet known
     * @param $time Content timestamp
     */
    public function __construct(\Course $course, \User $user=null, $time) {
        $this->course = $course;
        if($user !== null) {
            $this->userId = $user->get_id();
            $this->userName = $user->get_name();
            $this->userRole = $user->get_role();
        } else {
            $this->userId = null;
            $this->userName = null;
            $this->userRole = \User::STUDENT;
        }
     ...
     * @return User User object for this interaction
     */
    ...
    protected $course;  ///< The Course object

    protected $time;    ///< Content timestamp

    private $html = '';
    private $edits = array();
    private $endorsements = array();
}